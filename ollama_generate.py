...
Usage:
    python ollama_generate.py <jd_text_file> <Company Name> [model]
"""
...
FIXED_EXPERIENCE = [
    {"company": "Seek AI", "location": "New York, NY (Remote)", "dates": "Nov 2023 – Feb 2026",
     "seniority": "current, most recent — staff/senior level, must most heavily reflect the JD's responsibilities", "bullet_count": 9},
    {"company": "Medallion Bank", "location": "Murray, UT (Onsite)", "dates": "May 2019 – Aug 2023",
     "seniority": "senior level", "bullet_count": 7},
    {"company": "Nomi Health", "location": "Orem, UT (Onsite)", "dates": "Sep 2018 – Mar 2019",
     "seniority": "intern level", "bullet_count": 5},
]

FIXED_EDUCATION = {"school": "Weber State University", "degree_line": "Bachelor’s Degree, Computer Science, Apr 2014 – Sep 2018"}
...
def call_ollama(prompt: str, model: str) -> str:
    resp = requests.post(
        ...
        # well under a minute even on CPU, so 120s is enough headroom for
...
def _normalize_skills(skills):
    ...
    if all(isinstance(item, (list, tuple)) and len(item) == 2 for item in skills):
        return [[str(a), str(b)] for a, b in skills]
...
def build_summary_prompt(jd_text: str) -> str:
    return f"""You are a resume-tailoring engine. Output ONLY valid JSON, no markdown fences, no commentary.

Analyze the job description below and produce these 2 fields for an 8+ year experienced candidate named Jimmy Tran:
...
def build_skills_prompt(jd_text: str) -> str:
    return f"""You are a resume-tailoring engine. Output ONLY valid JSON, no markdown fences, no commentary.
...
def build_role_prompt(jd_text: str, spec: dict, avoid_bullets: list) -> str:
    avoid_block = ""
    if avoid_bullets:
...
Write a resume work-experience entry for Jimmy Tran at a role that is {spec['seniority']}.
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python ollama_generate.py <jd_text_file> <Company Name> [model]")
        sys.exit(1)
    ...
    print(f"Using local Ollama model '{model}' — generating in pieces (this may take a few minutes on CPU).")

    total_steps = 2 + len(FIXED_EXPERIENCE)

    print(f"  [1/{total_steps}] subtitle + summary...")
    summary_piece = generate_piece(build_summary_prompt(jd_text), model, ["subtitle", "summary"])

    print(f"  [2/{total_steps}] skills + keywords...")
    skills_piece = generate_piece(build_skills_prompt(jd_text), model, ["skills", "keywords"])

    overview = {**summary_piece, **skills_piece}

    experience = []
    all_prior_bullets = []
    for i, spec in enumerate(FIXED_EXPERIENCE, start=3):
        print(f"  [{i}/{total_steps}] {spec['company']} role...")
        role = generate_piece(build_role_prompt(jd_text, spec, all_prior_bullets), model, ["title", "bullets"])
    ...
    data_path = os.path.join(data_dir, f"{company.lower().replace(' ', '_')}_ollama.json")
    ...
    pdf_path = os.path.join(company_dir, "Jimmy Tran.pdf")
